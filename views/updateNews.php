...
class updateNewsView{
    public static function render($news,$allCategory){
        ?>
  ...
  <main class="hoc container clear"> 
    ...
    <form action="adminController.php" method="post" enctype="multipart/form-data">
        ...
        <label for="title">Inter the title </label>
         <input type="text" value = "<?php echo $news->title ?>" name="title" id="title" class="borderedbox inspace-10" required>

         <label for="date">Select your date:</label>
        <input type = "date" name="date" id="date"   value = "<?php echo $news->date ?>"  class="borderedbox2 inspace-10" required>
         

            <label for="category">Choose a category:</label>
        <select name="category" id="category">
             <?php foreach($allCategory as $x){?>
                <?php if($x->id == $news->category_id){?>
                    <option value="<?php echo $x->id?>" selected><?php echo $x->name?></option>
                    <?php }else{?>
        ...
        <textarea name="post" id="post"   class="borderedbox2 inspace-10" required> <?php echo $news->post ?></textarea>

        <label for="fileToUpload">Select image to upload </label>
        <input type="file" name="image" id="image" class="borderedbox inspace-10">
